...
New packages for this week include
...
install.packages(c("tidyverse", "dplyr", "ggraph", "tidytext", "gutenbergr", "janeaustenr", "tm", "wordcloud", "topicmodels", "text2vec", "ggplot2"))
```
...
Ascribing sentiment from text without having a human read it is difficult.
...
Nevertheless, people try to analyze the sentiment of text. The **tidytext** package includes a `data.frame` call `sentiments`
...
Although fairly useless, you can also make wordclouds
```{r}
library(wordcloud)
...
```
perhaps it would be better to organize them by sentiment, as shown in the reading.

# Federalist Papers

he text of the Federalist Papers need to be in a subdirectory called federalist below your working directory.
The Federalist Papers can be downloaded and unzipped via
```{r}
